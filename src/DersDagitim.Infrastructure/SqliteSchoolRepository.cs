...
public sealed class SqliteSchoolRepository : ISchoolRepository
{
    ...
    public async Task InitializeAsync(CancellationToken token = default)
    {
        ...
        command.CommandText = """
            ...
            CREATE TABLE IF NOT EXISTS ClassTeacherAssignments (ClassId TEXT PRIMARY KEY, TeacherId TEXT NOT NULL, AcademicYear TEXT NOT NULL, UpdatedAt TEXT NOT NULL);
            ...
            CREATE TABLE IF NOT EXISTS TeacherAvailabilityRestrictions (Id TEXT PRIMARY KEY, TeacherId TEXT NOT NULL, Day INTEGER NOT NULL, LessonNumber INTEGER NOT NULL, Type INTEGER NOT NULL, Note TEXT NULL, Severity INTEGER NOT NULL, ValidFrom TEXT NULL, ValidUntil TEXT NULL, RepeatsWeekly INTEGER NOT NULL DEFAULT 1, ReasonLabel TEXT NULL, UpdatedAt TEXT NOT NULL);
    ...
    public Task<IReadOnlyList<AvailabilityRestriction>> GetAvailabilityRestrictionsAsync(CancellationToken t = default) => ReadAsync("SELECT Id,TeacherId,Day,LessonNumber,Type,Note,Severity,ValidFrom,ValidUntil,RepeatsWeekly,ReasonLabel FROM TeacherAvailabilityRestrictions ORDER BY TeacherId,Day,LessonNumber", r => new AvailabilityRestriction(Guid.Parse(r.GetString(0)), Guid.Parse(r.GetString(1)), (DayOfWeek)r.GetInt32(2), r.GetInt32(3), (RestrictionType)r.GetInt32(4), r.IsDBNull(5) ? null : r.GetString(5), (AvailabilitySeverity)r.GetInt32(6), r.IsDBNull(7) ? null : DateOnly.Parse(r.GetString(7)), r.IsDBNull(8) ? null : DateOnly.Parse(r.GetString(8)), r.GetInt32(9) == 1, r.IsDBNull(10) ? null : r.GetString(10)), t);
    public Task SaveAvailabilityRestrictionAsync(AvailabilityRestriction value, CancellationToken t = default) => WriteAsync("INSERT INTO TeacherAvailabilityRestrictions (Id,TeacherId,Day,LessonNumber,Type,Note,Severity,ValidFrom,ValidUntil,RepeatsWeekly,ReasonLabel,UpdatedAt) VALUES ($id,$teacher,$day,$lesson,$type,$note,$severity,$from,$until,$repeat,$reason,$updated) ON CONFLICT(Id) DO UPDATE SET TeacherId=$teacher,Day=$day,LessonNumber=$lesson,Type=$type,Note=$note,Severity=$severity,ValidFrom=$from,ValidUntil=$until,RepeatsWeekly=$repeat,ReasonLabel=$reason,UpdatedAt=$updated", t, ("$id", value.Id.ToString()), ("$teacher", value.TeacherId.ToString()), ("$day", (int)value.Day), ("$lesson", value.LessonNumber), ("$type", (int)value.Type), ("$note", value.Note ?? (object)DBNull.Value), ("$severity", (int)value.Severity), ("$from", value.ValidFrom?.ToString("yyyy-MM-dd") ?? (object)DBNull.Value), ("$until", value.ValidUntil?.ToString("yyyy-MM-dd") ?? (object)DBNull.Value), ("$repeat", value.RepeatsWeekly ? 1 : 0), ("$reason", value.ReasonLabel ?? (object)DBNull.Value), ("$updated", DateTimeOffset.Now.ToString("O")));
    public Task DeleteAvailabilityRestrictionAsync(Guid id, CancellationToken t = default) => WriteAsync("DELETE FROM TeacherAvailabilityRestrictions WHERE Id=$id", t, ("$id", id.ToString()));
    public Task<IReadOnlyList<ClassTeacherAssignment>> GetClassTeacherAssignmentsAsync(CancellationToken t = default) => ReadAsync("SELECT ClassId,TeacherId,AcademicYear FROM ClassTeacherAssignments ORDER BY AcademicYear,ClassId", r => new ClassTeacherAssignment(Guid.Parse(r.GetString(0)), Guid.Parse(r.GetString(1)), r.GetString(2)), t);
    public Task SaveClassTeacherAssignmentAsync(ClassTeacherAssignment value, CancellationToken t = default) => WriteAsync("INSERT INTO ClassTeacherAssignments (ClassId,TeacherId,AcademicYear,UpdatedAt) VALUES ($class,$teacher,$year,$updated) ON CONFLICT(ClassId) DO UPDATE SET TeacherId=$teacher,AcademicYear=$year,UpdatedAt=$updated", t, ("$class", value.ClassId.ToString()), ("$teacher", value.TeacherId.ToString()), ("$year", value.AcademicYear), ("$updated", DateTimeOffset.Now.ToString("O")));
    public Task DeleteClassTeacherAssignmentAsync(Guid classId, CancellationToken t = default) => WriteAsync("DELETE FROM ClassTeacherAssignments WHERE ClassId=$class", t, ("$class", classId.ToString()));
